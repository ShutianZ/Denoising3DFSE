...
class Denoise3D(nn.Module):
    def __init__(self,in_channels=4,out_channel=2, n_channels=32,depth=4,kernel_size=3, stride=1, padding=1):
        super(Denoise3D, self).__init__()
        self.common1 = DM(in_channels,n_channels*2,depth,kernel_size,stride,padding)
        self.M14_1 = nn.Conv3d(in_channels = n_channels*2, out_channels = out_channel, kernel_size = kernel_size, padding = padding,bias=True)
        self.M14_2 = nn.Sequential(
            nn.Conv3d(in_channels = out_channel, out_channels = n_channels, kernel_size = kernel_size, padding = padding, bias=True),
            nn.ReLU(inplace = True)
        )
        self.M19 = nn.Sequential(
            nn.Conv3d(in_channels = n_channels*2, out_channels = n_channels, kernel_size = kernel_size, padding = padding, bias=True),
            nn.BatchNorm3d(n_channels, eps=0.0001, momentum = 0.95),
            nn.ReLU(inplace = True)
        )
        self.common2 = DoubM(n_channels*2,out_channel,depth,kernel_size,stride,padding)
...
class DM(nn.Module):
    def __init__(self,in_channels,n_channels,depth,kernel_size, stride, padding):
        ...
        self.stride = stride
        layers = []
        layers.append(nn.Conv3d(in_channels = in_channels, out_channels = n_channels, kernel_size = kernel_size, stride = stride, padding = padding, bias = True,dilation=1))
        layers.append(nn.ReLU(inplace=True))
        for _ in range(depth):
            layers.append(nn.Conv3d(in_channels = n_channels, out_channels = n_channels, kernel_size = kernel_size, padding = padding, bias=True,dilation=1))
...
class DoubM(nn.Module):
    def __init__(self,n_channels,out_channel,depth,kernel_size, stride, padding):
        ...
        layers.append(nn.Conv3d(in_channels = n_channels, out_channels = out_channel, kernel_size = kernel_size, padding = padding,bias=True,dilation=1))
